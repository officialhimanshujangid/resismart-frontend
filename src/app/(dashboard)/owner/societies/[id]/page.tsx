...
import {
  ArrowLeft, MapPin, CheckCircle2, XCircle, CreditCard, X, Download, Calendar, UserCheck, Globe, Phone, Mail,
} from 'lucide-react';
...
export default function SocietyDetailsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [detail, inv] = await Promise.all([
        ...
        api.get(`/billing/invoices?isPagination=true&pageSize=20&societyId=${id}`).then((r) => r.data.invoices || []).catch(() => []),
      ]);
      setSociety(detail.data.society);
  ...
  // Live prorated preview when picking a plan for an active society.
  useEffect(() => {
    if (!cashOpen || !cashForm.planId) { setPreview(null); return; }
    let cancelled = false;
    setPreviewLoading(true);
    api.post('/billing/upgrade-preview', { societyId: id, planId: cashForm.planId, tenure: cashForm.tenure })
  ...
  const approve = async () => {
    const ok = await confirm({ title: 'Approve Society', message: `Approve "${society.name}"? This provisions the admin and starts the free trial.`, confirmText: 'Approve', cancelText: 'Cancel', severity: 'info' });
    if (!ok) return;
    setBusy(true);
    try { await api.post(`/societies/${id}/approve`); showToast('Society approved', 'success'); load(); }
  ...
  const submitReject = async () => {
    setBusy(true);
    try { await api.post(`/societies/${id}/reject`, { reason: rejectReason }); showToast('Society rejected', 'success'); setRejectOpen(false); setRejectReason(''); load(); }
  ...
  const submitCash = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const payload: any = { societyId: id, planId: cashForm.planId, tenure: cashForm.tenure, paymentMethod: cashForm.paymentMethod, note: cashForm.note || undefined };
  ...
  if (!society) return <div className="text-center py-32 text-slate-400">Society not found.</div>;

  const coords = society.location?.coordinates;

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <button onClick={() => router.back()} className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-[#0a5bd7]"><ArrowLeft className="w-3.5 h-3.5" /> Back to societies</button>
          ...
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2.5 flex-wrap">
                <h1 className="text-2xl font-black text-slate-800 tracking-tight truncate">{society.name}</h1>
                <span className={`text-[10px] uppercase px-2.5 py-1 rounded-full font-black border ${STATUS_STYLES[society.status]}`}>{society.status}</span>
                <ModuleScope scope="society" />
              </div>
              <p className="text-sm text-slate-500 mt-0.5 truncate">{society.address}</p>
            </div>
          </div>
      ...
      {society.rejectionReason && (
        <div className="text-sm text-red-700 bg-red-50 border border-red-100 rounded-xl p-3">Rejection reason: {society.rejectionReason}</div>
      )}
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper elevation={0} className="p-6 rounded-2xl border border-slate-200/60 space-y-4">
            <h2 className="text-sm font-black uppercase tracking-wider text-slate-500">Society Information</h2>
            <Grid container spacing={2}>
              <Detail label="City" value={society.city} />
              <Detail label="State" value={society.state} />
              <Detail label="Pincode" value={society.pincode} />
              <Detail label="Registration No." value={society.registrationNumber} />
              <Detail label="Total Blocks" value={society.totalBlocks != null ? String(society.totalBlocks) : undefined} />
              <Detail label="Total Flats" value={society.totalFlats != null ? String(society.totalFlats) : undefined} />
            </Grid>

            <div className="pt-3 border-t border-slate-100">
              <h2 className="text-sm font-black uppercase tracking-wider text-slate-500 mb-2">Admin Contact</h2>
              <div className="space-y-1.5 text-sm">
                <p className="flex items-center gap-2 text-slate-700"><UserCheck className="w-4 h-4 text-slate-400" /> {society.contactName || '—'}</p>
                <p className="flex items-center gap-2 text-slate-700"><Mail className="w-4 h-4 text-slate-400" /> {society.contactEmail || '—'}</p>
                <p className="flex items-center gap-2 text-slate-700"><Phone className="w-4 h-4 text-slate-400" /> {society.contactPhone || '—'}</p>
                {society.website && <p className="flex items-center gap-2 text-slate-700"><Globe className="w-4 h-4 text-slate-400" /> {society.website}</p>}
              </div>
            </div>

            {coords?.length === 2 && (
              <div className="pt-3 border-t border-slate-100">
                <h2 className="text-sm font-black uppercase tracking-wider text-slate-500 mb-2">Location</h2>
                ...
                <a href={`https://www.google.com/maps/search/?api=1&query=${coords[1]},${coords[0]}`} target="_blank" rel="noreferrer" className="block text-xs font-bold text-[#0a5bd7] hover:underline mt-1">Open in Google Maps →</a>
              </div>
            )}
            ...
            <div className="pt-3 border-t border-slate-100 space-y-2 text-xs">
              ...
              <AuditRow label="Created by" value={society.createdByName} sub={fmt(society.createdAt)} />
              <AuditRow label="Updated by" value={society.updatedByName} sub={fmt(society.updatedAt)} />
            </div>
          </Paper>
        </Grid>
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          ...
          {upcoming.length > 0 && (
            <Paper elevation={0} className="p-6 rounded-2xl border border-blue-100 bg-blue-50/30 mt-4">
      ...
      {subscription?.history?.length > 0 && (
        <div>
          <h2 className="text-lg font-black text-slate-800 mb-3">Subscription Activity</h2>
          <Paper elevation={0} className="p-6 rounded-2xl border border-slate-200/60">
      ...
      {/* Invoices (past payments) */}
      <div>
        <h2 className="text-lg font-black text-slate-800 mb-3">Invoices & Past Payments</h2>
        <TableContainer component={Paper} elevation={1} className="rounded-2xl border border-slate-200/60 shadow-sm overflow-x-auto">
          {invoices.length === 0 ? (
            <div className="text-center py-12 text-slate-400 font-semibold text-sm bg-white">No invoices for this society.</div>
          ) : (
      ...
      <Dialog open={rejectOpen} onClose={() => setRejectOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <p className="text-sm text-slate-500 mb-3">Provide a reason for rejecting <strong>{society.name}</strong>.</p>
        ...
        {onlinePending ? (
          <>
            <DialogContent className="space-y-4 text-center py-8">
              ...
              <p className="text-sm text-slate-500">A payment link was emailed to the society. The plan will activate automatically the moment they pay.</p>
        ...
        <form onSubmit={submitCash}>
          <DialogContent className="space-y-3">
            ...
            {cashForm.paymentMethod === 'cash' ? (
              ...
              <p className="text-xs text-slate-500 bg-amber-50 border border-amber-100 rounded-lg p-2.5">A secure Razorpay payment link will be emailed to the society. The plan activates automatically once they pay — this window will update.</p>
            )}
