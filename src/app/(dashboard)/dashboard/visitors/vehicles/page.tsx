  ...
  const load = async () => {
    try {
      const [v, f] = await Promise.all([
        ...
        api.get('/visitors/flats').catch(() => ({ data: { data: [] } })),
      ]);
      setRows(v.data?.data || []);
  ...
  const columns: ColumnDef<Vehicle>[] = [
    ...
    },
    {
      ...
      render: v => v.parkingSlot
        ? <Chip size="small" label={v.parkingSlot} sx={{ bgcolor: '#f1f5f9', color: '#475569' }} />
    ...
    },
    {
      ...
      render: v => (
        <Button size="small" color="error" startIcon={<Trash2 className="w-3.5 h-3.5" />}
          onClick={() => remove(v)}>Remove</Button>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-4 pb-24">
      <PageHeader
        ...
        actions={
          <Button variant="contained" startIcon={<Plus className="w-4 h-4" />} onClick={() => setAdding(true)}>Add vehicle</Button>
        }
      />
