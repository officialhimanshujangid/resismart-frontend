...
import {
  Button, TextField, Select, MenuItem, FormControl, InputLabel, Alert, Checkbox,
...
export default function BlocklistPage() {
  ...
  const [lifting, setLifting] = useState<Row | null>(null);
  ...
  const load = async () => {
    try {
      const [b, c] = await Promise.all([
        api.get(`/visitors/blocklist?all=${showLifted}`),
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const res = await api.post('/visitors/blocklist', form);
  ...
  /**
   * Taking somebody off the list, in their own words.
   *
   * This used to post a hardcoded `reason: 'Lifted by committee'` — a sentence
   * nobody wrote, on a permanent record, attributed to a committee that was
   * never asked. Adding somebody to the list demands a reason and two
   * signatures; taking them off wrote fiction. Whoever lifts it now says why,
   * and the committee that follows theirs can read it.
   */
  const lift = async () => {
    if (!lifting) return;
    if (!liftReason.trim()) return showToast('Please say why — this is kept permanently', 'error');
    setSaving(true);
    try {
      await api.post(`/visitors/blocklist/${lifting._id}/lift`, { reason: liftReason.trim() });
      showToast('Removed from the list', 'success');
      setLifting(null); setLiftReason('');
      await load();
    } catch (e: any) {
  ...
  const columns: ColumnDef<Row>[] = [
    ...
    },
    {
      ...
      render: r => r.isActive
        ? <StatusChip status="ON_LIST" />
        : <Tooltip title={r.liftedReason || ''}>
            <span><StatusChip status="LIFTED" /></span>
          </Tooltip>,
    },
    {
      id: 'act', label: '', align: 'right', alwaysVisible: true,
      render: r => r.isActive ? (
        <Button size="small" color="inherit" startIcon={<Undo2 className="w-3.5 h-3.5" />}
          onClick={() => { setLifting(r); setLiftReason(''); }}>Take off the list</Button>
      ) : null,
    },
  ];

  if (loading) return <PageSkeleton label="Loading the list" />;

  return (
    <div className="space-y-4 pb-24">
      <PageHeader
        breadcrumb="Operations · Visitor Management"
        title="Not allowed inside"
      ...
      <DataTable
        ...
        columnToggle
        emptyTitle="Nobody is on the list"
        emptyText="That is the normal state. Add somebody from an entry in the visitor log when the committee has decided to."
        emptyIcon={<ShieldBan className="w-6 h-6" />}
        toolbar={
          <>
            <TextField placeholder="Search the list" value={q} onChange={e => setQ(e.target.value)}
            ...
            <Button size="small" variant={showLifted ? 'contained' : 'outlined'}
              ...
              {showLifted ? 'Showing people taken off too' : 'Show people taken off'}
            </Button>
          </>
        }
      />
      ...
      <AppDialog
        ...
        title="Add somebody to the list"
        busy={saving}
        confirmColor="error"
        confirmText={saving ? 'Saving…' : 'Add to the list'}
        onConfirm={save}
      >
        <FormControl fullWidth>
          <InputLabel>Match on</InputLabel>
          <Select label="Match on" value={form.basis}
            onChange={e => setForm({ ...form, basis: e.target.value as 'PHONE' | 'VEHICLE' })}>
            <MenuItem value="PHONE">Phone number</MenuItem>
            <MenuItem value="VEHICLE">Vehicle number</MenuItem>
          </Select>
        </FormControl>
        <TextField fullWidth label={form.basis === 'PHONE' ? 'Phone number' : 'Vehicle number'}
          value={form.value} onChange={e => setForm({ ...form, value: e.target.value })}
          placeholder={form.basis === 'PHONE' ? '9876543210' : 'MH 12 AB 1234'}
          helperText="It must already appear somewhere in the visitor log." />
        <TextField fullWidth label="Name (for the list only)" value={form.label}
          onChange={e => setForm({ ...form, label: e.target.value })}
          helperText="Shown to whoever reads the list. Never used to match anybody." />
        <TextField fullWidth multiline minRows={2} label="Why" value={form.reason}
          ...
          <Select multiple label="Agreed with" value={form.approverUserIds}
            ...
            {members.map(m => (
              <MenuItem key={m.userId} value={m.userId}>
                <Checkbox size="small" checked={form.approverUserIds.includes(m.userId)} sx={{ p: 0, mr: 1 }} />
      ...
      <AppDialog
        open={!!lifting}
        onClose={() => { setLifting(null); setLiftReason(''); }}
        title={`Take ${lifting?.label || lifting?.value || 'them'} off the list?`}
        busy={saving}
        confirmText={saving ? 'Saving…' : 'Take off the list'}
        confirmDisabled={!liftReason.trim()}
        onConfirm={lift}
      >
        <p className="text-sm text-slate-600">
          The gate stops being warned about them. The record of the original block stays,
          and so does this.
        </p>
        <TextField autoFocus fullWidth multiline minRows={2} label="Why are they coming off?"
          value={liftReason} onChange={e => setLiftReason(e.target.value)}
          helperText="Kept permanently, next to the reason they were added. Say who decided and when — “Committee meeting, 12 July: matter settled” reads far better in three years than “lifted”." />
      </AppDialog>
    </div>
  );
}
