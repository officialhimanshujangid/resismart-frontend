...
export default function GatesPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing._id) await api.put(`/gate/gates/${editing._id}`, body);
      else await api.post('/gate/gates', body);
  ...
  const retire = async (g: Gate) => {
    ...
    try {
      await api.post(`/gate/gates/${g._id}/retire`, {});
  ...
  const columns: ColumnDef<Gate>[] = [
    ...
    },
    {
      ...
      render: g => g.isActive
        ? <Chip size="small" label="In use" className="!bg-emerald-50 !text-emerald-700 !font-bold !text-[10px]" />
        : <Chip size="small" label="Retired" className="!bg-slate-100 !text-slate-500 !font-bold !text-[10px]" />,
    },
    {
      ...
      render: g => (
        <div className="flex items-center justify-end gap-1">
          <Button size="small" onClick={() => setEditing(g)}
            className="!rounded-xl !normal-case !font-bold !text-xs !text-slate-500">Edit</Button>
          {g.isActive && (
            <Button size="small" color="error" onClick={() => retire(g)}
              className="!rounded-xl !normal-case !font-bold !text-xs">Retire</Button>
          )}
        </div>
      ),
    },
      ...
      <PageHeader
        ...
        actions={
          <Button variant="contained" startIcon={<Plus className="w-4 h-4" />}
            onClick={() => setEditing({ kind: 'MAIN', handlesEntry: true, handlesExit: true, isActive: true })}
            className="!rounded-xl !normal-case !font-bold">Add gate</Button>
        }
      />
